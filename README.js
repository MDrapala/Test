// tools/convert/golangToJSON.js (v2)
// Convertit un dump "façon Go" (map[...], {...}, nil, etc.) en JSON.
// Usage : node tools/convert/golangToJSON.js <inputPath> [outputPath]
...
// Accumule un scalaire (mots non quotés avec espaces) jusqu’à la prochaine clé, ] ou }
...
function parseValue(it) {
  ...
  if (t.type === 'punct' && t.value === '{') {
    ...
    // Décider : objet (clé:valeur) OU tuple (val1 val2 ...)
    // Si les deux prochains tokens forment "word :", on part en mode objet.
    const isObject = isNextKey(it);
